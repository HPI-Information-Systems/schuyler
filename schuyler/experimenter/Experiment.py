...
class Experiment:
    def __init__(self, name, database_name, solution, database, sql_file_path, schema_file_path, groundtruth_path,hierarchy_level, tag, use_wandb=False, seed=42, iteration=0):
        wandb_mode_default = "online" if use_wandb else "disabled"
        tags = [tag] if tag else []

        init_kwargs = {
            "project": os.getenv("WANDB_PROJECT", "schuyler"),
            "mode": os.getenv("WANDB_MODE", wandb_mode_default),
            "tags": tags,
            "entity": os.getenv("WANDB_ENTITY", "Lasklu"),
            "dir": os.getenv("WANDB_DIR", "/tmp/models"),
            "config": {
    ...
    def run(self, solution_config):
        ...
        output, inference_time = self.solution.test(**test_config, sql_file_path=self.sql_file_path, schema_file_path=self.schema_file_path,model=trained_model, seed=self.seed, iteration=self.iteration)
