...
class SchuylerSolution(BaseSolution):
    ...
    def test(self, no_of_hierarchy_levels,min_max_normalization_sim_matrix, finetune,triplet_generation_model, similar_table_connection_threshold, model, groundtruth=None):
        ...
        train_dataset = tm.enrich_triplets(triplets)
...
#0.46
        ...
        cluster_dict = {}
        embedding_dict = {}
        for i, cluster in enumerate(node_clusterings[0]):
            #clustername = 
            cluster_name = hashlib.md5(str(cluster).encode()).hexdigest()
            cg.add_node(cluster_name)
            cluster_dict[cluster_name] = cluster
            encoding = []
            for table in cluster:
                node = G.get_node(table)
                encoding.append(node.encoding.astype(np.float32))
            embedding_dict[cluster_name] = np.mean(encoding, axis=0)
            # for table in cluster:
            #     cg.add_edge(cluster_name, table)
        # add edge if clusters have tables that have foreign key

        for table in self.database.get_tables():
            fks = table.get_foreign_keys()
            for fk in fks:
                for cluster in node_clusterings[0]:
                    if table.table_name in cluster:
                        for cluster2 in node_clusterings[0]:
                            if fk["referred_table"] in cluster2:
                                sim = util.cos_sim(embedding_dict[hashlib.md5(str(cluster).encode()).hexdigest()], embedding_dict[hashlib.md5(str(cluster2).encode()).hexdigest()]).item()
                                cg.add_edge(hashlib.md5(str(cluster).encode()).hexdigest(), hashlib.md5(str(cluster2).encode()).hexdigest(), weight=sim)
        louvain = louvain_clustering(cg)#, "weight")
        print("Final clustering", louvain)
        output = []
        for cluster in louvain:
            output.append([table for c in cluster for table in cluster_dict[c]])
        print("BVefore comdet", node_clusterings[0])
