...
import pickle
import argparse
import schuyler.solutions.tuta.reader as rdr
import schuyler.solutions.tuta.tokenizer as tknr
...
class WikiDataset(object):
    ...
    def build_and_save(self, workers_num):
        ...
        if workers_num < 1:
            raise ValueError("workers_num should be positive numbers")
        if workers_num == 1:
            print("HDKJAHDWKaAA", table_num)
            #raise ValueError("workers_num should be positive numbers")
            #print("Table lis", table_list)
...
def main():
    ...
    print(args)                         
    args.source_type = "wiki"
    args.input_path = "/data/..."
    args.output_path = "/data/dataset.pt"
