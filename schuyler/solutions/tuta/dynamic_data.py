...
import random
import pickle
from schuyler.solutions.tuta.utils import UNZIPS
from schuyler.solutions.tuta.tokenizer import PAD_ID
...
class DynamicDataLoader(object):
            ...
            while len(self.buffer) < self.buffer_size:
                ...
                while len(chunk) < self.chunk_size:
                    try:
                        ...
                        #print(self.f_reads[set_index])
                        tables = pickle.load(self.f_reads[set_index])
                        chunk.extend(tables)   
                if len(chunk) == 0 and not self.repeat_read_dataset:
                    break
    ...
    def sift_and_prep(self, instances):
        ...
        for ins in instances:
            ...
            # print("Matrix shape: ", np.shape(token_matrix))
            ...
            if (results is None) or (len(results[0]) > self.max_cell_num):
                print("Results too long. Still adding to buffer")
                continue
            token_seq = [tok for cell in results[0] for tok in cell]
            if len(token_seq) > self.max_seq_len:
                print("Token seq too long. Still adding to buffer")
                #continue
    ...
    def load_data(self):
        ...
        for (tok_list, num_list, pos_list, fmt_list, cell_ind, cell_mlm, cell_clc, cell_tcr) in self.buffer:
        ...
        for isample,_ in enumerate(self.buffer):
